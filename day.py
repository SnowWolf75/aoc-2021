...
# Date = 12/12/2021
# From https://github.com/SnowWOlf75/aoc-2021
# Based on the code from "init.py" in https://github.com/AlexeSimon/adventofcode
...
parser = argparse.ArgumentParser(description='Optionally pull down previous days. Usually just today.')
parser.add_argument('day', help='Day to pull down data for (remember the time difference)',
                    type=int, nargs='?')
...
y = datetime.datetime.now(tz=est).year
print("Year " + str(y))
...
if args.day:
    ...
    d = datetime.datetime.now(tz=est).day
...
if DOWNLOAD_INPUTS and (not os.path.exists(day_pos + "/input.txt") or OVERWRITE) and USER_SESSION_ID != "":
    ...
    while not done:
        try:
            with requests.get(url=link + str(y) + "/day/" + str(d) + "/input",
...
if DOWNLOAD_STATEMENTS and (not os.path.exists(day_pos + "/statement.html") or OVERWRITE):
    ...
    while (not done):
        try:
            with requests.get(url=link + str(y) + "/day/" + str(d), cookies={"session": USER_SESSION_ID},
                              headers={"User-Agent": USER_AGENT}) as response:
                if response.ok:
                    html = response.text
